...
exports.createSettings = asyncHandler(async (req, res) => {
  ...
  if (existing) throw new ApiError(409, "Website settings already exist. Use update instead.");

  const settings = await WebsiteSettings.create({ siteKey: "primary", ...req.body });
...
exports.getSettings = asyncHandler(async (_req, res) => {
  ...
  if (!settings) {
    return res.status(200).json({ success: true, settings: null, message: "Website settings not configured yet" });
  }
...
exports.updateLogo = asyncHandler(async (req, res) => {
  ...
  if (!logoUrl) throw new ApiError(400, "logoUrl is required");

  const existing = await WebsiteSettings.findOne({ siteKey: "primary" });

  if (!existing) {
    const created = await WebsiteSettings.create({ siteKey: "primary", siteName: "Tara Maa Solutions", logoUrl });
    setImmediate(() => log(req, { action: "logo_updated", category: "settings", details: "Uploaded site logo", resourceName: "Logo" }));
  ...
  setImmediate(() => log(req, { action: "logo_updated", category: "settings", details: "Updated site logo", resourceName: "Logo" }));
...
exports.deleteSettings = asyncHandler(async (_req, res) => {
  ...
  if (!settings) throw new ApiError(404, "Website settings not found");
